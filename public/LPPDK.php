...
if ($arr['jnspencalonan_id'] == 1){   
	$inputFileName = "template/LPPDK_TEMPLATE.xls";
}else { 
	$inputFileName = "template/LPPDK_TEMPLATE_PERSEORANGAN.xls";
}
...
$objPHPExcel->setActiveSheetIndex(5)
            ->setCellValue('B2', $arr['jenis_pilkada'])
            ->setCellValue('B3', $arr['nama_daerah'])
            ->setCellValue('B4', $arr['jenis_pencalonan'])
            ->setCellValue('B5', 'PERIODE: ' . $arr['periode_lppdk'])
            ->setCellValue('B6', $arr['tanggal_lppdk'])
...
$result = $db->query('SELECT * FROM v_pengeluaran_lppdk');
...
$result = $db->query('SELECT * FROM v_penerimaan_lppdk');
...
$activeSheet = $objPHPExcel->getActiveSheet();
if($arr['jnspencalonan_id'] == 2) {
	$activeSheet->setTitle("LPPDK5-PERSEORANGAN");
}
//protect_sheet($activeSheet, $password);
...
$result = $db->query('SELECT * FROM v_penerimaan_lppdk_5');
...
$result = $db->query('SELECT sum(total) as t_total, sum(t_uang) as tt_uang, sum(t_barang) as tt_barang, sum(t_jasa) as tt_jasa FROM v_penerimaan_lppdk_5');
...
header('Content-Disposition: attachment;filename="LPPDK-'.$arr['JENIS_PENCALONAN'].'.xlsx"');
